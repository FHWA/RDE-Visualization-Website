...
function barChart(data, chartDiv, parentDiv, userOptions) {
  ...
  var chart = {},
      optionDefaults = {
        ...
        height: 220 - 0 - 40,
        margins: {
          top: 0, right: 10, bottom: 40, left: 30,
        },
        eventFlag: false,
        yDomain: null,
        yLabel: '%',
        timeBegin: d3.min(data, function (d) { return d.StartTime; }),
        timeEnd: d3.max(data, function (d) { return d.EndTime; }),
        expandFlag: false,
        leftColumn: true,
  ...
  chart.renderChart = function (newWidth, newHeight) {
    ...
    chart.rects.transition()
      .duration(900)
      .ease("quad-out")
      .attr("x", function (d) { return chart.xScale(d.StartTime); })
      ...
      .attr("height", function (d) { return chart.options.height - chart.yScale(d.Value); })
  ...
  chart.brushChart = function () {
    ...
    chart.rects.attr("x", function (d) { return chart.xScale(d.StartTime); })
      .attr("y", function (d) { return chart.yScale(d.Value) })
      .attr("width", function (d) {
        if (chart.xScale(d.EndTime) - chart.xScale(d.StartTime) > 0 && chart.xScale(d.EndTime) - chart.xScale(d.StartTime) < 1) {
             return 1
